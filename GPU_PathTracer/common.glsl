...
Material createPlasticMaterial(vec3 albedo, vec3 specClr, float percentSpecular, float roughness)
{
    Material m;
    m.type = MT_METAL;
    m.albedo = albedo;

    m.specColor = percentSpecular * specClr;
    m.roughness = roughness;
    m.emissive = vec3(0.0);
    return m;
}
...
bool scatter(Ray rIn, HitRecord rec, out vec3 atten, out Ray rScattered)
{
    ...
    if (rec.material.type == MT_DIELECTRIC) {
        ...
        //TODO: MT_PLASTIC algumas coisas entram e voltam a sair, o schlick é usado para calcular a qtt de luz que é refletida vs refratada
...
QuadLight createQuadLight(Quad q, vec3 color, float intensity)
{
    QuadLight l;
    l.quad = q;
    l.color = color;
    ...
    if (c > 0.0f) { //origin is outside sphere
        if (b <= 0.0f)
            ...
            if (d <= 0.0f)
                ...
                t = b - sqrt(d);
                if (s.radius < 0.0) 
                    normal = normalize(s.center -r.o + r.d * t);
                else 
...
//TODO: Russian Roulette - prob da funcao da throughput vai diminuindo, quando o throguhput for probabilistaicmanete inferior a um n
// predefinido, podemos acabar os loops
// VER TEORICA
